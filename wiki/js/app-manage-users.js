...
export class appManageUsers extends LitElement {
    ...
    checkBlocked(user) {
        if (user.blocked) {
            return html`
                <fa-icon title="Blokkeer gebruiker" @click="${(e) => this.toggleBlocked(user, e.target)}"
                         class="fas fa-ban blocked"></fa-icon>`
        } else {
            return html`
                <fa-icon title="Blokkeer gebruiker" @click="${(e) => this.toggleBlocked(user, e.target)}"
                         class="fas fa-ban"></fa-icon>`
        }
    }
    ...
    mapStudent(user) {
        return html`
            <span id="icon-holder">
                <fa-icon title="Maak gebruiker auteur"
                         @click="${() => this.toggleToAuthor(user).then(() => this.requestUpdate())}"
                         class="fas fa-pencil-alt"></fa-icon>
                ${this.checkBlocked(user)}
                <fa-icon title="Verwijder gebruiker" @click="${() => this.deleteUser(user)}"
    ...
    mapAuthor(user) {
        return html`
            <span id="icon-holder">
                <fa-icon title="Maak gebruiker student"
                         @click="${() => this.toggleToStudent(user).then(() => this.requestUpdate())}"
                         class="fas fa-pencil-alt author"></fa-icon>
                <fa-icon title="Maak gebruiker admin"
                         @click="${() => this.toggleToAdmin(user).then(() => this.requestUpdate())}"
                         class="fas user-shield"></fa-icon>
                ${this.checkBlocked(user)}
                <fa-icon title="Verwijder gebruiker" @click="${() => {
    ...
    render() {
        return html`
            ...
            <h3>Legenda:</h3>
            <p> <fa-icon class="fas user-shield admin"></fa-icon>  Maak gebruiker admin<br>
            ...
            <div id="flex-container">
                <ul class="usercontainer">
                    ...
                    ${this._students.map((user) => html`
                        <li>${user.fullName}
                            ${this.mapStudent(user)}
                        </li>
                    `)}
                </ul>
                <ul class="usercontainer">
                    <h2>Auteurs</h2>
                    ${this._authors.map((user) => html`
                        <li>${user.fullName}
                            ${((user.email === 'admin@tester.nl' || user.email === 'auteur@tester.nl') ? html`` : html` <!--Students and authors can't see this page. -->
                            ${this.mapAuthor(user)}`)}
                        </li>
                    `)}
                </ul>
